...
void playertermination(float centipede1[12][6], float player[],int gameGrid[][gameColumns]);
void centipede_headshot(float centipede1[12][6],float bullet[],int gameGrid[][gameColumns]);
  ...
  if (!font.loadFromFile("Roboto-Bold.ttf")) {
   ...
   sf::Text scoreText;
    ...
    while (window.isOpen())
    {
        ...
        playertermination( centipede1, player,gameGrid);
	...
	if (bullet[exists])
	{
	    ...
	    centipede_headshot(centipede1,bullet, gameGrid);
	}
	...
	void movecentipede(float centipede1[12][4], sf::Clock& CentipedeClock)
	{
		if (CentipedeClock.getElapsedTime().asMilliseconds() < 80)
			return;
		...
		for (int i = 0; i < 12; i++)
		{
			if (!centipede1[i][2]) // not exists
				continue;

			int currX = (int)centipede1[i][0];
			int currY = (int)centipede1[i][1];

			if (i == 0) // head segment
			{
				bool& moveLeft = *(bool*)&centipede1[i][3];
				int nextX = moveLeft ? currX - 32 : currX + 32;
				int gridX = nextX / boxPixelsX;
				int gridY = currY / boxPixelsY;

				// Mushroom or wall collision
				if (nextX < 0 || nextX >= resolutionX || gameGrid[gridX][gridY] == 1 || gameGrid[gridX][gridY] == 7)
				{
					moveLeft = !moveLeft;
					centipede1[i][1] += 32; // move down
				}
				else
				{
					centipede1[i][0] = nextX;
				}
			}
			else // body segment
			{
				centipede1[i][0] = prevX;
				centipede1[i][1] = prevY;
			}

			// Store this segment's position for the next one
			prevX = currX;
			prevY = currY;
		}
	}
